...
router.get("/", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ detail: "Erro interno no servidor ao buscar produtos." });
  }
});
...
router.post("/", upload.single("imagem"), async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ detail: "Erro interno no servidor ao adicionar produto." });
  }
});
...
router.delete("/:id", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ detail: "Erro interno no servidor ao remover produto." });
  }
});
